...
from .models import PromoCode, Category, Color, Size, Product
...
class ProductDetailView(APIView):
    def get(self, request, category, sex, id):
        product = get_object_or_404(Product, category__name=category, sex=sex, id=id)
        reviews = ReviewProduct.objects.filter(good=product)

        return Response(self.get_product_details(product, reviews), status=status.HTTP_200_OK)

    def get_product_details(self, product, reviews):
        review_data = [ 
            {
...
class ProductsSearchView(APIView):
    def get(self, request):
        ...
        return Response(
            {"message": "No query provided or no results found"},
    ...
    try:
        if promo.start_date <= timezone.now() <= promo.end_date:
            if promo.usage_limit > promo.times_used:
                ...
                promo.save()
                return Response(
                    {"success": True, "discount": promo.discount_value},
            ...
            return Response(
                {"success": False, "message": "promo code is not valid"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    except:
