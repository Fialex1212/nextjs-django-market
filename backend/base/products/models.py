...
class PromoCode(models.Model):
    DISCOUNT_TYPES_CHOICES = [("fixed", "fixed"), ("percentage", "percentage")]
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    code = models.CharField(unique=True, max_length=10)
    discount_type = models.CharField(
        max_length=20, choices=DISCOUNT_TYPES_CHOICES, default="percentage"
    )
...
class Product(models.Model):
    ...
    image = models.ImageField(
        upload_to="uploads/products/", max_length=255, blank=True, null=True
    )
    images = models.ManyToManyField(Image, related_name="products", blank=True)
    rating = models.IntegerField(
        validators=[MinValueValidator(0.0), MaxValueValidator(5.0)], default=5
    )
